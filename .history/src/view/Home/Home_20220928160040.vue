...
export default {
  setup() {
    ...
    function goInteriorDecoration(e:any){
      console.log(e)
      router.push({
...
<template>
  ...
  <div id="main">
    <el-row justify="center">
      <el-col @click="item.gotoPage" data-name="item.menuName" v-model="item.menuName" class="menu-col" v-for="item in menulist" :span="11">
        <div class="menu">
          <div class="submenu">
            <img class="menu-logo" :src="item.logoUrl" alt="" />
            <div>
              {{ item.menuName }}
              <div style="font-size: 12px">{{ item.menuAlias }}</div>
            </div>
          </div>
        </div>
      </el-col>
